...
class CompilationErrorTest extends FlatSpec with Matchers {
  ...
  it should "not match different exact type mismatch errors" in {
    ...
    val err = TypeMismatchError(RegexT.fromPattern("com.softwaremill.String"), None,
      RegexT.fromPattern("com.softwaremill.RequiredType[String]"), None, None)
    ...
    val nonMatchingErrs = List(
      ...
      TypeMismatchError(ExactT("com.softwaremill.Int"), None, ExactT("com.softwaremill.RequiredType[String]"), None, None)
    )
  ...
  it should "match regex in type mismatch errors" in {
    // given
    val err = TypeMismatchError(RegexT.fromPattern("slick.dbio.DBIOAction[*]"), None,
      RegexT.fromPattern("slick.lifted.Rep[Option[*]]"), None, None)
    val matchingErrs = List(
      TypeMismatchError(ExactT("slick.dbio.DBIOAction[Unit,slick.dbio.NoStream,slick.dbio.Effect.Write]"), None,
        ExactT("slick.lifted.Rep[Option[?]]"), None, Some("notes")),
      TypeMismatchError(ExactT("slick.dbio.DBIOAction[String,slick.dbio.NoStream,slick.dbio.Effect.Read]"), None,
        ExactT("slick.lifted.Rep[Option[Int]]"), None, None),
      TypeMismatchError(ExactT("slick.dbio.DBIOAction[Unit,slick.dbio.NoStream,slick.dbio.Effect.Read]"), None,
        ExactT("slick.lifted.Rep[Option[Option[Int]]"), None, None)
    )
    val nonMatchingErrs = List(
      TypeMismatchError(ExactT("slick.dbio.DBIOAction[Unit,slick.dbio.NoStream,slick.dbio.Effect.Read]"), None, ExactT("String"), None, Some("notes")),
...
class CompilationErrorProperties extends Properties("CompilationError") {
  property("obj -> json -> obj works for type mismatch error") =
    forAll { (found: String, foundExpandsTo: Option[String], required: String, requiredExpandsTo: Option[String]) =>
      val e = TypeMismatchError(RegexT.fromPattern(found), foundExpandsTo.map(RegexT.fromPattern),
        RegexT.fromPattern(required), requiredExpandsTo.map(RegexT.fromPattern), None)
  ...
  property("matches identical type mismatch error") = forAll { (found: String, required: String, notes: Option[String]) =>
    TypeMismatchError(RegexT.fromPattern(found), None, RegexT.fromPattern(required), None, None).matches(
      TypeMismatchError(ExactT(found), None, ExactT(required), None, notes))
  }
}
