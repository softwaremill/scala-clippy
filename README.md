...
````scala
addSbtPlugin("com.softwaremill.clippy" % "plugin-sbt" % "0.3.3")
````
...
````scala
addCompilerPlugin("com.softwaremill.clippy" %% "plugin" % "0.3.3" classifier "bundle")
````

If you are using `scalac` directly, add the following option:

````scala
-Xplugin:clippy-plugin_2.11-0.3.3-bundle.jar
````
